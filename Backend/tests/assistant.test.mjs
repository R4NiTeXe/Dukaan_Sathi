...
test('empty question returns 400 before calling Gemini', async () => {
  const r = await request(baseUrl, 'POST', '/api/v1/assistant/ask', { token, body: { question: '' } });
...
intentTest('answers today summary with revenue data', { timeout: 60000 }, async () => {
  await request(baseUrl, 'POST', '/api/v1/billing/save', {
    token,
    body: { items: [{ productName: 'Rice', quantity: 2, unit: 'kg', price: 200 }], paymentMethod: 'cash' },
  });
...
intentTest('answers weekly comparison question', { timeout: 60000 }, async () => {
  const r = await request(baseUrl, 'POST', '/api/v1/assistant/ask', {
    token,
...
intentTest('answers Bengali question in Bengali', { timeout: 60000 }, async () => {
  const r = await request(baseUrl, 'POST', '/api/v1/assistant/ask', {
    token,
    body: { question: '\u0986\u099c\u0995\u09c7\u09b0 \u09ac\u09bf\u0995\u09cd\u09b0\u09bf \u0995\u09c7\u09ae\u09a8?' },
  });
  assert.equal(r.status, 200, r.json?.message);
  assert.equal(r.json.data.intent, 'today');
  assert.ok(r.json.data.answer.length > 0);
...
intentTest('answers Hindi question', { timeout: 60000 }, async () => {
  const r = await request(baseUrl, 'POST', '/api/v1/assistant/ask', {
    token,
    body: { question: '\u0906\u091c \u0915\u0940 \u092c\u093f\u0915\u094d\u0930\u0940 \u0915\u0948\u0938\u0940 \u0939\u0948?' },
  });
  assert.equal(r.status, 200, r.json?.message);
  assert.equal(r.json.data.intent, 'today');
  assert.ok(r.json.data.answer.length > 0);
...
test('assistant answers when Gemini is up, graceful 502 when down', { timeout: 60000 }, async () => {
  const r = await request(baseUrl, 'POST', '/api/v1/assistant/ask', {
    token,
    body: { question: 'today summary' },
  });
