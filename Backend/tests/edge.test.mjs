...
test('empty DB analytics return empty arrays, not errors', async () => {
  const monthly = await request(baseUrl, 'GET', '/api/v1/analytics/monthly', { token });
  assert.equal(monthly.status, 200);
  assert.deepEqual(monthly.json.data.data, []);

  const top = await request(baseUrl, 'GET', '/api/v1/analytics/top-products', { token });
  assert.equal(top.status, 200);
  assert.deepEqual(top.json.data.data, []);

  const customers = await request(baseUrl, 'GET', '/api/v1/analytics/customer-report', { token });
...
test('non-existent bill returns 404', async () => {
  const r = await request(baseUrl, 'GET', '/api/v1/bills/507f1f77bcf86cd799439011', { token });
...
test('oversized request body returns 413', async () => {
  const big = {
    items: [{ productName: 'X'.repeat(1.2 * 1024 * 1024), quantity: 1, unit: 'piece', price: 10 }],
    paymentMethod: 'cash',
  };
  const r = await request(baseUrl, 'POST', '/api/v1/billing/save', { token, body: big });
  assert.equal(r.status, 413);
});

test('concurrent bill saves produce unique sequential numbers', async () => {
  const bodies = Array.from({ length: 10 }, (_, i) => ({
    items: [{ productName: `Item ${i}`, quantity: 1, unit: 'piece', price: 10 }],
    paymentMethod: 'cash',
  }));
  ...
  assert.equal(new Set(numbers).size, 10, `numbers must be unique, got: ${numbers}`);
  assert.deepEqual(
    numbers.sort(),
