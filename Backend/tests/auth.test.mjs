...
test('register creates a user and returns an access token', async () => {
  const r = await request(baseUrl, 'POST', '/api/v1/auth/register', {
    body: { ownerName: 'Auth Test', shopName: 'Auth Shop', shopType: 'grocery', email: EMAIL, password: PASSWORD },
  });
...
test('register rejects invalid input with 400', async () => {
  const r = await request(baseUrl, 'POST', '/api/v1/auth/register', {
    body: { ownerName: 'X', shopName: '', shopType: 'grocery', email: 'not-an-email', password: 'short' },
  });
...
test('access token works on protected routes', async () => {
  const r = await request(baseUrl, 'GET', '/api/v1/customers', { token: accessToken });
