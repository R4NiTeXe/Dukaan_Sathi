...
test('list customers with search', async () => {
  ...
  const s = await request(baseUrl, 'GET', '/api/v1/customers?search=rah', { token });
  assert.equal(s.json.data.customers.length, 1);

  const empty = await request(baseUrl, 'GET', '/api/v1/customers?search=zzz', { token });
...
test('bill numbers increment sequentially', async () => {
  const r = await request(baseUrl, 'POST', '/api/v1/billing/save', {
    token,
    body: { items: [{ productName: 'Sugar', quantity: 1, unit: 'kg', price: 50 }], paymentMethod: 'cash' },
  });
  assert.equal(r.status, 201);
...
test('save bill without customer keeps customerId null', async () => {
  const r = await request(baseUrl, 'POST', '/api/v1/billing/save', {
    token,
    body: { items: [{ productName: 'Tea', quantity: 1, unit: 'pack', price: 30 }], paymentMethod: 'upi' },
  });
  assert.equal(r.status, 201);
...
test('list bills with pagination and filters', async () => {
  const r = await request(baseUrl, 'GET', '/api/v1/bills?limit=2&page=1', { token });
  assert.equal(r.status, 200);
  ...
  const paid = await request(baseUrl, 'GET', '/api/v1/bills?paymentStatus=unpaid', { token });
  assert.equal(paid.json.data.pagination.total, 0);

  const cash = await request(baseUrl, 'GET', '/api/v1/bills?paymentMethod=cash', { token });
...
test('analytics customer-report includes customer details', async () => {
  const r = await request(baseUrl, 'GET', '/api/v1/analytics/customer-report', { token });
  assert.equal(r.status, 200);
...
test('delete bill returns 404 on second delete', async () => {
  const r = await request(baseUrl, 'DELETE', `/api/v1/bills/${billId}`, { token });
  assert.equal(r.status, 200);

  const again = await request(baseUrl, 'DELETE', `/api/v1/bills/${billId}`, { token });
  assert.equal(again.status, 404);
});

test('dashboard summary aggregates correctly', async () => {
  const r = await request(baseUrl, 'GET', '/api/v1/dashboard/summary', { token });
  assert.equal(r.status, 200);
...
test('analytics monthly returns current month entry', async () => {
  const r = await request(baseUrl, 'GET', '/api/v1/analytics/monthly', { token });
  assert.equal(r.status, 200);
...
test('analytics top-products ranks by revenue', async () => {
  const r = await request(baseUrl, 'GET', '/api/v1/analytics/top-products', { token });
  assert.equal(r.status, 200);
...
test('delete customer', async () => {
  const r = await request(baseUrl, 'DELETE', `/api/v1/customers/${customerId}`, { token });
  assert.equal(r.status, 200);
});

test('delete product', async () => {
  const r = await request(baseUrl, 'DELETE', `/api/v1/products/${productId}`, { token });
  assert.equal(r.status, 200);
});
